...
// Tambahkan table
    ...
    $sql = "Declare @store varchar(4) Declare @Date1 datetime   Declare @Date2 datetime                   
            ...
            select #LAKON MKG# as toko,PLU,Supplier_Code,name,Article_Code,item_description,cast(SUM(qty) as int)qty,price,SUM(Discount_Amount)Discount_Amount
            ,margin,## as prod,isppn,sum(hsl)hsl,convert(varchar(10),@Date1,120) tgl1, convert(varchar(10),@Date2,120) as tgl2
            from
            (
            select convert(varchar(10),DocDate,120)period,d.PLU,c.Supplier_Code,e.name,c.Article_Code 
            ,c.Long_Description as item_description,b.Quantity qty,d.Price-d.Discount_Amount-d.extraDisc_Amt Price,d.Discount_Amount,b.Margin as margin,#-# prod
